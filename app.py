...
def _get_history(ip):
    """Get conversation history for a session, max 10 most recent exchanges."""
    hist = _conversation_history.get(ip, [])
    return hist[-10:]  # Last 10 exchanges = up to 20 messages


def _add_to_history(ip, role, content):
    """Add a message to conversation history and trim."""
    if ip not in _conversation_history:
        _conversation_history[ip] = []
    _conversation_history[ip].append({"role": role, "content": content})
    if len(_conversation_history[ip]) > MAX_HISTORY_PER_SESSION:
        _conversation_history[ip] = _conversation_history[ip][-MAX_HISTORY_PER_SESSION:]
...
def api_ask():
    ...
    _history = _get_history(_session_ip)
    _history_str = ""
    if _history:
        _history_lines = []
        for msg in _history[-6:]:  # Last 3 exchanges
            role_label = "User" if msg["role"] == "user" else "Assistant"
            _history_lines.append(f"{role_label}: {msg['content'][:500]}")
        _history_str = "\n\n### Previous Conversation:\n" + "\n".join(_history_lines)

    # ── Try AI-generated response using research context ──
    # Also run when only medical terms match (no direct peptide name)
    _medical_terms = extract_medical_terms(question)
    if matched_peptides or _medical_terms:
        try:
            peptides = list(matched_peptides) if matched_peptides else []
            terms = _medical_terms if _medical_terms else None
